...
class ReplaceWarehouseTest {
    ...
    void replaceRejectsInvalidInput() {
        ...
        assertEquals(422, assertThrows(WebApplicationException.class,
                () -> uc.replace(null)).getResponse().getStatus());

        assertEquals(422, assertThrows(WebApplicationException.class,
                () -> uc.replace(buildWarehouse("  ", "AMSTERDAM-001", 10, 5))).getResponse().getStatus());

        assertEquals(422, assertThrows(WebApplicationException.class,
                () -> uc.replace(buildWarehouse("BU-1", null, 10, 5))).getResponse().getStatus());

        assertEquals(422, assertThrows(WebApplicationException.class,
                () -> uc.replace(buildWarehouse("BU-1", "AMSTERDAM-001", 0, 5))).getResponse().getStatus());

        assertEquals(422, assertThrows(WebApplicationException.class,
                () -> uc.replace(buildWarehouse("BU-1", "AMSTERDAM-001", 10, -1))).getResponse().getStatus());
    }
}
